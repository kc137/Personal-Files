...
model.a1 = pyo.ConstraintList()
model.a2 = pyo.ConstraintList()
...
for m in model.m:
    for j in model.j:
        for k in range(model.j.at(j) + 1, model.j.at(-1) + 1):
            ...
            model.a2.add(start[m, disj_seq[m, k]] 
...
def disj1(model, a):
    d1 = dp.Disjunct()
    d1.cons = pyo.Constraint(expr = model.a1[a].expr)
    model.add_component(f"d1_{a}", d1)
    return d1

def disj2(model, b):
    d2 = dp.Disjunct()
    d2.cons = pyo.Constraint(expr = model.a2[b].expr)
