...
demdatapd = pd.DataFrame(demdata, index = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q'], columns = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q'])
...
model.time_window = pyo.Param(model.CUSTOMERS, domain = pyo.Any, initialize = {
    ...
    'M' : (0, 5), 'N' : (5, 10), 'O' : (7, 8), 'P' : (10, 15), 'Q' : (5, 15)})
...
def service(model, i, j, k):
    if i != j:
        return (s[i, k] + times[j][i] - M*(1 - x[i, j, k]) <= s[j, k] - 0.000001)
    else:
        return s[i, k] == s[i, k]
...
for k in model.VEHICLES:
    ...
    route = [f"{(i, j)}->{model.time_window[j]}, {times[j][i]}" for i in model.CUSTOMERS
              for j in model.CUSTOMERS
