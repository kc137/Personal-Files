...
model.capacity = pyo.Param(model.VEHICLES, initialize = {'V1': 15, 'V2': 15, 'V3' : 15, 'V4' : 15, 'V5' : 15})
...
def objective_rule(model):
    return sum(model.distance[j][i]*model.x[i, j, k] if j != i else 0
               ...
               for j in model.CUSTOMERS
                for i in model.CUSTOMERS) + sum(model.x[i, j, "V"+str(V)]*M - 
                model.distance[j][i]*(model.x[i, j, "V"+str(V)]) 
                for j in model.CUSTOMERS
...
for i in model.CUSTOMERS:
    for j in model.CUSTOMERS:
        if model.x[i, j, "V"+str(V)]() and model.x[i, j, "V"+str(V)]() > 0.9 and (i != "A1" or j != "A1"):
            dropped["pts"].add(i) if i != "A1" else dropped["pts"].add(j)
print(f"Dropped customers: {dropped['pts']}")

print(f"Total distance with penalties : {model.objective()}")