...
for d in NRD:
    for t in days:
        model.c1.add(
            pyo.quicksum(
                pctUndamaged*pctFromToD[d2, d]*pctRent[r]*tr[d2, (t-r)%6] for d2, r in dr
                ) + 
            pyo.quicksum(
                tu[d2, d, (t-1)%6] for d2 in depots if d2 != d
                ) + 
            eu[d, (t-1)%6] == nu[d, t]
            )
...
for d in NRD:
    for t in days:
        model.c3.add(
            pyo.quicksum(
                pctDamaged*pctFromToD[d2, d]*pctRent[r]*tr[d2, (t-r)%6] for d2, r in dr
                ) + 
            ed[d, (t-1)%6] == nd[d, t]
            )
...
for d in RD:
    for t in days:
        model.c4.add(
            ...
            pyo.quicksum(
                td[d, d2, (t-1)%6] for d2, dd in d2notd if dd == d
                ) + 
            ed[d, (t-1)%6] == nd[d, t]
            )
...
model.c5.add(
    pyo.quicksum(
        (0.25*tr[d, 0] + 0.45*tr[d, 1] + nu[d, 2] + nd[d, 2] for d in depots)) == n
    )
