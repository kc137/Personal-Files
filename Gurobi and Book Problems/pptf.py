...
model.x = pyo.Var(fabrics, looms, within = pyo.NonNegativeReals)
x = model.x
...
def time_cons(model):
    return (pyo.quicksum(x[f, "J"] / loom_speed_j[f-1] 
                         for f in fabrics) 
            + pyo.quicksum(x[f, "R"] / loom_speed_r[f-1] 
                           for f in fabrics 
                           if f >= 3) 
            <= 720)
model.c1 = pyo.Constraint(rule = time_cons)
...
def demand_cons(model, f):
    if f >= 3:
        ...
        return pyo.quicksum(x[f, l] for l in looms 
                            if l != "R") >= demands[f-1]
model.c2 = pyo.Constraint(fabrics, rule = demand_cons)
...
# sol = SolverFactory("glpk", executable = "F:\\Solvers\\couenne\\bin")
