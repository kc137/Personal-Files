...
def consecutive_shifts(model, n, d, s):
    if s != model.S.at(-1):
        return x[n, d, s] + x[n, d, model.S.next(s)] <= 1
    else:
        return pyo.Constraint.Skip
model.c1 = pyo.Constraint(model.N, model.D, model.S, rule = consecutive_shifts)
...
# nurse_shift_allocation = []

# for n in model.N:
#     for d in model.D:
#         for s in model.S:
#             nurse_shift_allocation.append(round(x[n, d, s]()))